...
def splitFiles(zeroTime,fromDay,lastDay):
	...
	for day in range(fromDay,lastDay):
		...
		for hour in range(25):
			fromRangeTime = 13858524 + 36*hour + 36*24*(day-1)		
			for sixMinutes in range(6):
				mapToLowRangeTimeValue[zeroTime] = fromRangeTime
				zeroTime = zeroTime + 6
		finalDict = defaultdict(list)
		print (str(day).zfill(2))
		csv_data = csv.reader(file("sms-call-internet-mi-2013-12-"+str(day).zfill(2)+".txt"))
		for row in csv_data:
			rowtab = row[0].split('\t')
...
splitFiles(13858524,1,10)
splitFiles(13866300,10,20)
splitFiles(13874940,20,32)